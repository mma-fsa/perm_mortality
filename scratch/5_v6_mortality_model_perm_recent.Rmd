...
group_model_data <- function(df) {
  df %>%
    mutate(attained_age = pmin(95, attained_age)) %>%
...
exp_basis_formula <- number_of_deaths ~ 
  sm_ind * splines::bs(attained_age, 
...
hin <- function(x) {
  ...
  # ineq_vals[8] <- x[8] - x[7]
  # ineq_vals[9] <- x[9] - x[8]
  # ineq_vals[10] <- x[10] - x[9]
  ineq_vals[11] <- x[11] - x[10]
  ineq_vals
}
...
X_attage <- splines::bs(test_attage, 
                              Boundary.knots = c(25, 95), 
                              knots = c(35, 40, 45, 52, 70, 80),
                       degree=3) 
