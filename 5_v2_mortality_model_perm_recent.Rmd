...
bit_str_to_mm <- function(bit_str, model_data) {
  ...
  ga_df <- model_data %>%
    mutate(
      ...
      ns_ind = 1 - sm_ind,
      AGE_GRPS_NS = cut(attained_age, breaks = curr_ns_breaks),
      AGE_GRPS_SM = cut(attained_age, breaks = curr_sm_breaks),
...
bit_str_to_mm(best_model@solution[1, ], model_data.train)$ga_params
...
best_mm <- bit_str_to_mm(best_model@solution[1, ], model_data.train)
