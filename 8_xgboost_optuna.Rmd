...
objective_fn <- carrier::crate(
  function(trial) {
    ...
    optuna_cb <- function(trial, metric_name="poisson-nloglik") {
      xgb_cb <- function(env = parent.frame()) {
        
        browser()
        
        i <- env$iteration
        score <- env$bst_evaluation[1]
      ...
      attr(xgb_cb, "name") <- "cb.early.stop.optuna"
    ...
    xgb_model <- xgboost::xgb.train(
      ...
      callbacks = list(optuna_cb(trial)),
...
function (stopping_rounds, maximize = FALSE, metric_name = NULL, 
    verbose = TRUE) 
{
    best_iteration <- -1
    best_ntreelimit <- -1
    ...
    callback <- function(env = parent.frame(), finalize = FALSE) {
        ...
        if ((maximize && score > best_score) || (!maximize && 
            score < best_score)) {
            best_msg <<- .format_eval_string(i, env$bst_evaluation, 
